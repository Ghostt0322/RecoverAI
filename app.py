...
"""
    ...
    # Make one request per analysis to conserve the model's daily quota.
    try:
...
with st.sidebar:
    ...
    if st.button("Reset demo"):
        ...
        st.session_state.audit = []
        st.session_state.recovered_amount = 0
        for key in ["latest_result", "latest_case", "latest_used_ai", "last_selected_case"]:
...
# "Recovered" is tracked explicitly so the metric remains correct after a simulated action.
# Recovery rate is measured against the original demo revenue at risk.
recovered = int(st.session_state.get("recovered_amount", 0))
original_at_risk = int(sum(
    r["amount"] for r in DATA if r["status"] == "failed"
))
recovery_rate = (recovered / original_at_risk * 100) if original_at_risk else 0
...
with tab2:
    ...
    if st.session_state.get("last_selected_case") != case_id:
        for key in ["latest_result", "latest_case", "latest_used_ai"]:
            st.session_state.pop(key, None)
    ...
    with right:
        ...
        if "latest_result" in st.session_state and st.session_state.get("latest_case", {}).get("id") == case_id:
            ...
            already_recovered = row["status"] == "recovered"

            if already_recovered:
                st.info("This payment has already been recovered in this demo run.")
            elif action not in {"NO_ACTION", "ESCALATE_TO_HUMAN"}:
                if st.button("Approve recovery action"):
                    idx = df.index[df["id"] == case_id][0]

                    # Record the simulated recovered amount exactly once.
                    recovery_amount = int(row["amount"])
                    st.session_state.df.loc[idx, "status"] = "recovered"
                    st.session_state.recovered_amount += recovery_amount

                    st.session_state.audit.append({
                        "time": datetime.now().strftime("%H:%M:%S"),
                        "payment": case_id,
                        "event": "RECOVERY_APPROVED",
                        "details": f"{action}; simulated recovery ₹{recovery_amount:,.0f}"
                    })

                    st.success(
                        f"Simulated recovery completed: ₹{recovery_amount:,.0f}. "
