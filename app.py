...
Analyze this failed payment and return ONLY valid JSON with keys:
action, confidence, expected_recovery, reasoning.
Allowed actions: RETRY, REMINDER_THEN_RETRY, SEND_REMINDER, PAYMENT_METHOD_UPDATE,
ESCALATE_TO_HUMAN, NO_ACTION.
Rules: never invent facts; never claim a real payment was recovered; confidence is 0-100;
expected_recovery must not exceed the payment amount; explain 2-4 concrete reasons.
...
"""
        ...
        # Gemini can occasionally wrap JSON in markdown fences or add a short preface.
        if "```" in raw:
            parts = raw.split("```")
            ...
            if raw.lower().startswith("json"):
                raw = raw[4:].strip()
        # Extract the JSON object if any extra text remains.
        start, end = raw.find("{"), raw.rfind("}")
        if start == -1 or end == -1 or end <= start:
            raise ValueError("Gemini returned no valid JSON object")
        result = json.loads(raw[start:end + 1])

        allowed = {"RETRY", "REMINDER_THEN_RETRY", "SEND_REMINDER",
        ...
        expected = max(0, min(int(row["amount"]), int(float(result.get("expected_recovery", 0)))))
